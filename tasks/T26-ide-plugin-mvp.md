...
buildPlugin`) and #2 (runIde manual) — PENDING the maintainer (need network + emulator).
...
2. `./gradlew runIde` manual script (the maintainer): open `samples/single-module`, configure, Start →
