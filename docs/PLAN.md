...
| T41 | Narrow 0.2.0 release: stable configured-plugin/profile path, AI-assisted setup, aligned artifacts, and bounded packaged-artifact validation | large | 🚧 IN PROGRESS 2026-07-23 — host/documentation/version milestone (`092a3e0`), packaged AGP-8/JDK-17 and AGP-9/JDK-21 device lanes (`ba3b5a2`), configured single-module suite, configured capture suite, multi-module suite 4/4 twice, and clean-clone configured documentation gate all pass. T42 (`b890922`) fixed feature-module state loss caused by Compose debug-location hash noise. Remaining: maintainer production configured smoke, then authorized release actions. |
