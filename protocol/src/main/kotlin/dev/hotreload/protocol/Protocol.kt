...
sealed class Request(val requestId: Int) {
     ...
     * reflectively calls `LiveEditStubs.addClasses(classes, supportClasses, structural=false)`,
     * then the invalidate tail: `invalidateGroupsWithKey` per [groupIds] when non-empty, else
     * `invalidateAllCompositions()`. [supportClasses] (v8, T28) are lambda-shaped classes whose
     * constructor changed (e.g. Compose restart lambdas regenerated by a composable signature
     * change): they are registered proxy-flagged — interpreted `NEW` on them creates a
     * `Proxies.*` VM proxy instead of instantiating the (stale) loaded class — and are never
     * primed. [primedDexName] is informational (the InjectDex name of the
    ...
    class LiveEditClasses(
        ...
        val supportClasses: List<ClassBytes>,
